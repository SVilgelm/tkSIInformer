...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--init', action='store_true', dest='init', default=False)
    parser.add_argument('-d', '--database', dest='db', default=settings.DB)
    parser.add_argument('-u', '--use-proxy', action='store_true', dest='use_proxy', default=False)
    ...
    parser.add_argument('-a', '--add-author', action='append', dest='add_authors', default=[])
    ...
    try:
        ...
        if args.show:
            ...
            for author in sorted(models.Author.get(), key=lambda author: author.name):
                if only_authors:
                    print('{name:>s}: {url:>}'.format(name=author.name, url=author.url))
                else:
                    books = models.Book.get_by_author(author, only_new=only_new)
                    if books:
                        print('{name:>s}: {url:>}'.format(name=author.name, url=author.url))
                        for book in books:
                            if book.is_new:
                                ...
                                template = '\t{name:>s}: {url:>}'
                            print(template.format(name=book.name, url=book.url))
